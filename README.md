...
현재 구현 milestone은 **V1.9T — Core Cooking Recipe Catalog Expansion**이다.
...
- Shared Material catalog: 64 active (기존 Carrack 9 포함)
- Cooking Recipe: Recipe 9 / IngredientGroup 7 / Members 35 / Slots 36 / Options 40 (모두 active, 9개 Recipe aggregate verified)
- 테스트: backend 460 passed / frontend 57 passed
...
- `/api/calculations/recipes/{slug}` strict `attempt_count` 기반 stateless Recipe batch API — 각 option을 독립 배율 계산하고 AND/OR와 IngredientGroup target을 보존
...
`write`는 generated tree를 동기화하고 stale 파일을 제거한다. `check`는 파일을 수정하지 않고 committed export가 canonical seed와 같은지 검사한다. 두 명령 모두 실제 `backend/bdo.db`와 개인 상태를 export source로 사용하지 않는다.
...
2026-09-11 완료 확인 기준 backend 테스트는 460 passed, frontend 테스트는 57 passed다.
...
Cooking Recipe 연속 milestone은 `handoff/V19Q_COOKING_RECIPE_FOUNDATION_REPORT.md`, `handoff/V19R_COOKING_RECIPE_EVIDENCE_CLOSURE_REPORT.md`, `handoff/V19S_RECIPE_BATCH_CALCULATION_REPORT.md`, `handoff/V19T_CORE_COOKING_RECIPE_CATALOG_REPORT.md`에 기록한다.
