...
def test_v17c_temp_db_migration_import_idempotence_and_history_preservation(tmp_path, monkeypatch) -> None:
    ...
    while True:
        ...
        if not dependent_slugs:
            break
        baseline_content_rows = [row for row in baseline_content_rows if row["slug"] not in dependent_slugs]    # Later content may reuse a source introduced with V1.7C. Keep every source
