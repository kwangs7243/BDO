...
## Life detail 추가 섹션
- V1.9A `/life`: 공통 생활 기반, 10개 생활 분야 진입점, 사용자 진행 요약, 검증 상태, 생활 기반·경제 Content를 표시한다.
- V1.9A `/life/:skill`: 기존 canonical Content를 분야 기반/시작/장비/핵심/반복/심화/경제로 묶고, 실제 데이터가 없는 빈 섹션은 숨긴다.
- 각 요약 카드는 기존 `/content/:slug` 상세를 재사용하고, 항해·물물교환의 관련 Project는 실제 Project-Content 연결이 있는 경우에만 `/projects/:slug`로 노출한다.
