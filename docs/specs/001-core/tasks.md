...
> V1.7C 완료 상태를 기준으로 현재 코드, seed, 테스트와 handoff에서 확인된 구현만 반영했다. 미완료 복합 항목에는 부분 구현 범위를 덧붙였으며, 이 문서는 V1.7D 로드맵을 정의하지 않는다.
...
- [ ] adventure logs
- [ ] Magnus — 원격 창고 관련 seed는 있으나 Magnus 전체 범위는 미완료
